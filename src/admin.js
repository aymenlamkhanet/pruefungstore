// ==========================================
// PRÜFUNGSTORE PRO — ADMIN ENGINE (ADMIN.JS)
// ==========================================
...
function switchTab(tabId) {
  ...
  document.querySelectorAll(".nav-tab-btn").forEach(btn => {
    if (btn.dataset.tab === tabId) {
      btn.className = "nav-tab-btn active flex items-center gap-3 px-4 py-3 rounded-2xl bg-amber-500/15 text-amber-300 font-bold border border-amber-500/30 shadow-lg text-xs sm:text-sm transition-all";
    } else {
      btn.className = "nav-tab-btn flex items-center gap-3 px-4 py-3 rounded-2xl text-slate-400 hover:text-white hover:bg-white/5 font-semibold text-xs sm:text-sm transition-all";
    }
  });
  ...
  if (tabId === "overview") { renderKPIs(); renderCharts(); }
  else if (tabId === "products") { renderProductsTable(); }
  else if (tabId === "orders") { renderOrdersTable(); }
  else if (tabId === "clients") { renderClientsTable(); }
  else if (tabId === "settings") { loadSettingsUI(); }
...
function renderKPIs() {
  ...
  if (revEl) revEl.textContent = totalRevenue + " DH";
  ...
  if (recentList) {
    ...
    recentList.innerHTML = recent.map(o => `
      <div class="flex items-center justify-between p-3.5 rounded-2xl bg-white/5 border border-white/5 hover:border-white/15 transition">
        <div class="flex items-center gap-3">
          <div class="w-10 h-10 rounded-xl bg-amber-500/10 text-amber-400 flex items-center justify-center font-bold text-xs">
            ` + o.id.slice(-4) + `
          </div>
          <div>
            <h4 class="font-outfit font-bold text-sm text-white">` + o.customerName + `</h4>
            <p class="text-[11px] text-slate-400">` + o.customerCity + ` • ` + o.items.length + ` article(s)</p>
          </div>
        </div>
...
function renderProductsTable() {
  ...
  let list = adminState.books.filter(b => {
    const matchFilter = filter === "all" || (filter === "B1" && b.level.includes("B1")) || (filter === "B2" && b.level.includes("B2")) || (filter === "pack" && b.isPack);
  ...
  tbody.innerHTML = list.map(b => `
    <tr class="border-b border-white/5 hover:bg-white/5 transition">
      <td class="p-4">
        <div class="flex items-center gap-3">
          <img src="` + b.image + `" alt="` + b.title + `" class="w-12 h-12 object-contain bg-black/60 rounded-xl p-1 border border-white/10">
          <div>
            <h4 class="font-outfit font-bold text-white text-sm line-clamp-1">` + b.title + `</h4>
            <span class="text-[11px] text-slate-400">` + b.id + ` • ` + b.examType + `</span>
          </div>
        </div>
      </td>
      <td class="p-4">
      ...
      <td class="p-4">
        <button onclick="toggleProductPack('` + b.id + `')" class="px-2 py-0.5 rounded-full text-[10px] font-bold ` + (b.isPack ? "bg-emerald-500/20 text-emerald-300 border border-emerald-500/30" : "bg-white/5 text-slate-400") + `">
          ` + (b.isPack ? "★ Pack" : "Solo") + `
        </button>
      </td>
  ...
  tbody.innerHTML = list.map(o => `
    <tr class="border-b border-white/5 hover:bg-white/5 transition">
      ...
      <td class="p-4">
        ...
        <span class="text-[11px] text-slate-400">` + o.customerCity + `</span>
      </td>
      <td class="p-4">
...
function exportDataJSON() {
  const data = { books: adminState.books, orders: adminState.orders, settings: adminState.settings, exportDate: new Date().toISOString() };
...
function getStatusBadgeClass(status) {
  const map = { nouveau: "bg-amber-500/20 text-amber-300 border border-amber-500/30", preparation: "bg-blue-500/20 text-blue-300 border border-blue-500/30", expedie: "bg-purple-500/20 text-purple-300 border border-purple-500/30", livre: "bg-emerald-500/20 text-emerald-300 border border-emerald-500/30", annule: "bg-red-500/20 text-red-300 border border-red-500/30" };
  return map[status] || "bg-white/10 text-white";
}

function getStatusLabel(status) {
  const map = { nouveau: "🟡 Nouvelle", preparation: "🔵 En Préparation", expedie: "🟣 Expédiée", livre: "🟢 Livrée (Payée)", annule: "🔴 Annulée" };
